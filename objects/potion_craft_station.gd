...
@onready var panel_21: Panel = $TwoIngredientUI/Panel2
...
@onready var panel_22: Panel = $ThreeIngredientUI/Panel2
...
func add_recipe_item_UI(item : PotionResource) -> void:
	if num_recipe_items == 0:
		...
		slot_1.set_recipe_item_data(item, 1)
		num_recipe_items += 1
	
	elif num_recipe_items == 1:
		if item.name == recipe1_item_pickup.name:
			recipe1_num += 1
			slot_1.increase_item_quantity(1)
			return
		else:
			one_ingredient_ui.hide()
			two_ingredient_ui.show()
			slot_3 = slot_2
			slot_4.set_recipe_item_data(item, 1)
		...
		if item.name == recipe1_item_pickup.name:
			...
			slot_5 = slot_3
			slot_6 = slot_4
			slot_7.set_recipe_item_data(item, 1)
